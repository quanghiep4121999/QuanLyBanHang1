...
import com.example.quanlybanhang.adapter.CartAdapter;
...
public class HoaDonChiTietActivity extends AppCompatActivity {
    ...
    public void ADDHoaDonCHITIET(View view) {
        ...
        sanPhamDAO = new SanPhamDAO(HoaDonChiTietActivity.this);
        try {
            if (validation() < 0) {
                Toast.makeText(getApplicationContext(), "Vui lòng nhập đầy đủ thông tin", Toast.LENGTH_SHORT).show();
            } else {
                SanPham sanPham = sanPhamDAO.getSanPhamByID(edMaSanPham.getText().toString());
                if (sanPham !=null){
                    int pos = checkMaSanPham(dsHDCT,edMaSanPham.getText().toString());
                    HoaDon hoaDon = new HoaDon(edMaHoaDon.getText().toString(),new Date());
                    HoaDonChiTiet hoaDonChiTiet = new HoaDonChiTiet(1,hoaDon, sanPham, Integer.parseInt(edSoLuong.getText().toString()));
                    if (pos>=0){
                    ...
                    Toast.makeText(getApplicationContext(),"Mã cá không tồn tại", Toast.LENGTH_SHORT).show();
                }
            }
        } catch (Exception ex) {
    ...
    public void thanhToanHoaDon(View view) {
        ...
        try {
            for (HoaDonChiTiet hd: dsHDCT) {
                ...
                thanhTien = thanhTien + hd.getSoLuongMua() * hd.getSanPham().getGia();
            }
        ...
        public int checkMaSanPham(List<HoaDonChiTiet> lsHD, String maSanPham){
            int pos = -1;
            for (int i = 0; i < lsHD.size(); i++){
                HoaDonChiTiet hd = lsHD.get(i);
                if (hd.getSanPham().getMaSanPham().equalsIgnoreCase(maSanPham)){
                    pos = i;
                    break;
                }
            }
        ...
        public int validation(){
            if (edMaSanPham.getText().toString().isEmpty()||edSoLuong.getText().toString().isEmpty()|| edMaHoaDon.getText().toString().isEmpty()){
                return -1;
            }
            return 1;
    }
